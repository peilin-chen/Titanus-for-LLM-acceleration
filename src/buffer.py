...
class Buffer(object):
	...
	Attributes:
		buffer_type (str): type of buffer in ['global', 'sz']
	...
	def __init__(self, buffer_type, config, constants):
		...
		self.leakage_power = constants[f'{buffer_type}_buffer']['leakage'] * config[f'{buffer_type}_buffer_size'] # mW
		self.dynamic_power = self.access_energy * constants['clock_frequency'] # mW
		self.area = constants[f'{buffer_type}_buffer']['area'] * config[f'{buffer_type}_buffer_size']
		self.data_width = config['data_width']
		self.quant_key = config['quant_bit']['key']
		self.quant_value = config['quant_bit']['value']
		self.main_memory_bandwidth = constants['main_memory']['bandwidth'][config['main_memory']['mode']]*1024 #(1024 is for reducing the simulation time.(do not influence the energy results))
		...
		if self.buffer_type == 'global':
			self.weight_factor = self.data_width
		elif self.buffer_type == 'sz':
			self.weight_factor = self.data_width + 1.0
