...
class AttentionCim(object):
	"""AttentionCim class
	
	Attributes:
		AttentionCim (str): name of the given AttentionCim
		cim_macros (list): list of CimMacro objects
		accumulator (Accumulator): Accumulator module object
	"""
	def __init__(self, attention_cim_name, config, constants):
		self.attention_cim_name = attention_cim_name

		self.cim_macros = []
		for n in range(math.ceil(config['hidden_size'] / 256) * math.ceil(config['hidden_size'] / 32)): # macro_size: [256, 32]
			self.cim_macros.append(CimMacro(f'{self.attention_cim_name}_CimMacro{(n + 1)}', config, constants))
		
		self.accumulators = []
		for n in range(config['hidden_size']):
			self.accumulators.append(Accumulator(f'{self.attention_cim_name}_Accumulator{(n + 1)}', config, constants))
